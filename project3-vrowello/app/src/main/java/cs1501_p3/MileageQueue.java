	...
	public void add(Car c) throws IllegalStateException {
		...
		while (index > 0) {
			if (index % 2 == 1) {
				index = (index-1)/2;
				Car swap = arr[index];
				if (c.getMileage() < swap.getMileage()) {
					arr[(2*index)+1] = swap;
	...
	public void updateMileage(String vin, int newMileage) throws NoSuchElementException {
		...
		while (index > 0) {
			if (index % 2 == 1) {
				index = (index-1)/2;
				Car swap = arr[index];
				if (car.getMileage() < swap.getMileage()) {	
					swapLeft(swap, index);
					car = arr[index];
				}
				else {
		...
		try {
			while (arr[(2*index)+1] != null) {
				if (arr[(2*index)+2] != null) {
					if (arr[index].getMileage() < arr[(2*index)+1].getMileage() && arr[index].getMileage() < arr[(2*index)+2].getMileage()) {
						break;
					}

					if (arr[(2*index)+1].getMileage() < arr[(2*index)+2].getMileage()) {
						swapLeft(car, index);
